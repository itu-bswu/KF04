...
In order to always choose the node closest to the source, our implementation use a priority queue, where its distance to the source indexes each node. Sometimes, we want to decrease the index of a node, because we have found a route that is shorter than the one previously indexed. Because the priority queue in javaÕs library is slow when decreasing keys, we use an implementation from the book Algorithms Fourth Editon.
We have written the code for the dijkstra algoritm using our knowledge of how it works. We have been inspired by the implementation used in Algorithms Fourth Editon.
...
>>>>>>> 0d962e1a1c5277660b452fbae28c01744d2d174e
...
We started for a way to speed up the loading process, so the user has a map in front of 
him or her quickly, when the user starts the program.
\pagebreak
\end{document}