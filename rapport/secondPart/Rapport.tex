...
the one previously indexed. Because the priority queue in javaÕs library is slow
...
In order to make our path finding algorithm even faster, we used a technique
called A-star. This technic benefits from the fact that the roads, in contrast
...
straight motorway, which often is far from possible. We can therefore conclude
that the A-star technique typically is more of a benefit when calculating routes
for bicycling than car driving.
