...
\section{Features}
\label{Feat}
Dette kapitel beskriver de features, vi har og ikke har valgt at implementere i
...
Det er muligt at zoome ind på en specific del af kortet ved at klikke med musen,
trække en ``kasse'' hen over det, man vil zoome ind på, og så slippe musen.
Billedet vil altid justere sig selv i forhold til applikationsvinduets
størrelse.

Dette betyder, at kortet alid vil vise det, som blev valgt, men nogle gange vil
...
Når man kører musen hen over en vej, så vises navnet på vejen i bunden af
applikationen.
...
at man trykker på Escape-tasten. Dette gør det nemmere at navigere kortet, hvis
man har zoomet langt ind.
\subsection{Vis færre veje, når man er zoomed ud}
...
med at gøre kortet hurtigere.
\subsection{Features, som ikke er med}
Den eneste måde, at bevæge sig rundt på kortet, er ved at anvende pilknapperne
på GUI. Vi overvejede at implementere både piltasterne og musen som
navigationsmulighed. Vi har gemt det væk for nu, da vi følte, at det var
``nice-to-have'' mere end en nødvendighed.

Vi har valgt ikke at gøre det mulig at vælge, hvilke typer veje man vil have
...
Lige som ovenstående billede viser, så har et grafisk vindue sin egen klasse.
De klasser bliver kaldt ``views''. Hvor man førhen havde en klasse til både at
...
Så mangler vi bare en måde at forbinde den grafiske bruger-grænseflade (``views'')
...
finder sted. Hvis der f.eks. så bliver opdateret noget data i det grafiske vindue,
så står controlleren så for at sende den nye data videre til modellen.
...
gør kortet meget hurtigere at nagivere rundt på.
...
\class{QuadTree} skal så vide hvilket zoomniveau, der skal vises. Den ville så
kun returnere de veje, som lå ned til den højde af træet, som zoomniveau havde
bestemt.
...
det så oveni feltet \field{bounds} koordinater.
...
information omkring tykkelse og farve af den pågældende vej. Tykkelsen og
...
indebærer af knapper til manøvrering på kortet, statusbar til visning af navnet på vejen
musen peger på samt selve kortet. Selve kortet (\class{Canvas}) er implementeret
som en privat indre klasse som udvider java's \class{Component}-klasse.
\class{Canvas} har mulighed for at tegne vores \class{Line} objekter og kan
derved tegne kortet uden nogen viden om den bagved-liggende data-struktur. Det
...
denne situation. Eftersom \class{View} ikke har til ansvar at fortolke brugerens
input, er der en lang række metoder til at tilføje ActionListeners til de forskellige
events der kommer, når brugeren bruger musen, ændrer på vinduets størrelse eller
...
\begin{figure}
\begin{center}
\includegraphics[angle=90, bb = 0 0 100 1500, scale = 0.2]{UTMillu}
\end{center}
