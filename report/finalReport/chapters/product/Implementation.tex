...
Dijkstras algorithm uses a minimum priority queue to find the shortest path
...
the target). See figure \ref{A*vsD} for an example. 
...
rectangle, and it will return all the roads within our smaller squares that
intersect that rectangle. This technique optimizes the drawing of the roads, because we
...
With this method we will not have to iterate over all the raods and remove the
them all the time.
...
\class{MD5Checksum}\footnote{We borrowed this code from \cite{MD5}}. When we serialize 
...
with this, as \class{nodes} and \class{edges} are both stored in the quadtrees
and the graph. If we serialized and saved the quadtrees in one file, and the
graph in other file, a given \class{node} will be saved twice, and when we load
the data from the serialized files, the \class{node} will exist twice, and it won't be
